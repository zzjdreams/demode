...
% %画图
subplot(221) 
plot(data,'.-');title('MSK 传 输 的 数 据 ');xlabel(' 时 间');ylabel(' 幅度')
subplot(222) 
plot(data_diff,'.-');title(' 差 分 后 的 数 据 ');xlabel(' 时 间');ylabel(' 幅度') 
subplot(223) 
plot(I1,'.-');title(' 加权前 I 路');xlabel(' 时间');ylabel(' 幅度'); 
subplot(224) 
plot(Q1,'.-');title(' 加权前 Q路');xlabel(' 时间');ylabel(' 幅度'); 
figure() 
subplot(221) 
plot(cos(pi*t/2/Tb),'.-');title(' 加 权 函 数 cos( πt/(2Tb))');xlabel(' 时间');ylabel(' 幅度') 
subplot(222) 
plot(sin(pi*t/2/Tb),'.-');title(' 加 权 函 数 sin( πt/(2Tb))');xlabel(' 时间');ylabel(' 幅度') 
subplot(223) 
plot(I_out,'.-');title(' 加权后 I 路');xlabel(' 时间');ylabel('幅度'); 
subplot(224) 
plot(Q_out,'.-');title(' 加权后 Q路');xlabel(' 时间');ylabel('幅度'); 
% %**************************************************************************
figure()
plot(signal_out,'.-');title(' 调制信号产生');xlabel(' 时间');ylabel(' 幅度')